...
int searchForInitializaion(Frame& refFrame, Frame& currFrame, int th, vector<int>& vMatches, std::vector<cv::Point2f>& vPrevMatched){
    ...
    for(size_t i = 0; i < vRefKeys.size(); i++){
        ...
        for(size_t j : vIndicesToCheck){
            ...
            if (vnMatches21[j] != -1 && dist >= vMatchedDistance[vnMatches21[j]]) {
                continue;
            }
        ...
        if(bestIdx >= 0 && bestDist <= th && (float)bestDist < (float(secondBestDist)*0.9)){
            if (vnMatches21[bestIdx] != -1) {
                vMatchedDistance[vnMatches21[bestIdx]] = 255;
...
int searchWithProjection(Frame& currFrame, int th, std::vector<std::shared_ptr<MapPoint>>& vMapPoints){
    ...
    for(shared_ptr<MapPoint> pMP : vMapPoints){
        ...
        int best_idx = -1;
        float best_dist = 255, second_best_dist = 255;
        ...
        for(auto j : vIndicesToCheck){
            ...
            int matchDist = HammingDistance(desc,currFrame.getDescriptors().row(j));
            if(matchDist < best_dist){
                best_dist = matchDist;
                best_idx = j;
            }
            else if(matchDist < second_best_dist){
                second_best_dist = matchDist;
            }
        }
        if(best_idx > -1 && best_dist < th && (float)best_dist < (float(second_best_dist)*0.9)) {
            currFrame.setMapPoint(best_idx, pMP);
            nMatches++;
        }

    }

    cout << "Proj: " << nMatches << endl;
    return nMatches;
}
